...
method  trajectories  can  be combined.  Furthermore any lambda -> lambda'
...
We  want  to  calculate  the  potential energy ``U(lambda) = Uenv + (1-lambda)**N  Ureac  +  lambda**N  Uprod``,  where  N  is positive integer
...
Uenv  only and having the perturbation energy routine calculated determine
